...
var ENTITY_ATTR_MAP = (_ENTITY_ATTR_MAP = {}, _defineProperty(_ENTITY_ATTR_MAP, _draftJsUtils.ENTITY_TYPE.LINK, { url: 'href', rel: 'rel', target: 'target', title: 'title', className: 'class' }), _defineProperty(_ENTITY_ATTR_MAP, _draftJsUtils.ENTITY_TYPE.IMAGE, {
  src: 'src',
  height: 'height',
  width: 'width',
  alt: 'alt',
...
var MarkupGenerator = function () {
  function MarkupGenerator(contentState, customStyles) {
    ...
    value: function renderBlockContent(block) {
      ...
      return entityPieces.map(function (_ref) {
        ...
        var content = stylePieces.map(function (_ref3) {
          ...
          if (style.has(BOLD) && !_this.customStyles[BOLD]) {
            content = '<strong>' + content + '</strong>';
          }
          if (style.has(UNDERLINE) && !_this.customStyles[UNDERLINE]) {
            content = '<ins>' + content + '</ins>';
          }
          if (style.has(ITALIC) && !_this.customStyles[ITALIC]) {
            content = '<em>' + content + '</em>';
          }
          if (style.has(STRIKETHROUGH) && !_this.customStyles[STRIKETHROUGH]) {
            content = '<del>' + content + '</del>';
          }
          if (style.has(CODE) && !_this.customStyles[CODE]) {
          ...
          try {
            for (var _iterator5 = Object.keys(_this.customStyles)[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
              var styleKey = _step5.value;

              var styleFunction = _this.customStyles[styleKey];
...
function stateToHTML(content, customStyles) {
